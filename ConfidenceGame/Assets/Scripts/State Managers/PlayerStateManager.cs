		...
		if (Network.isServer)
		{
			if (mySprite.transform.position.x < 13.36) {
				if (Input.GetKeyDown (KeyCode.Return)) {
					...
					mySprite.transform.position = new Vector3 (mySprite.transform.position.x + points*0.15f, mySprite.transform.position.y, mySprite.transform.position.z);
			...
			if (mySprite.transform.position.x < 13.36) {
				if (Input.GetKeyDown (KeyCode.Return)) {
					int points = textFieldHandler.instance.GetWorth();
					opponentSprite.transform.position = new Vector3 (opponentSprite.transform.position.x + points*0.15f, opponentSprite.transform.position.y, opponentSprite.transform.position.z);
					MainStateManager.Instance.SendPointsScored(points);
				}
			}
		}
